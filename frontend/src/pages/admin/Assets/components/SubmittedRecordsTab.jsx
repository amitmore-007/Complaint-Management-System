...
const SubmittedRecordsTab = ({ isDarkMode }) => {
  ...
  const { data: assetRecords = [], isLoading } = useAssetRecords();
  ...
  const filteredRecords = useMemo(() => {
    ...
    return assetRecords.filter((record) => {
      const store = String(record.storeName || "").toLowerCase();
      const tech = String(record.technician?.name || "").toLowerCase();
      return store.includes(q) || tech.includes(q);
    });
  }, [assetRecords, searchTerm]);

  const downloadAssetRecordsExcel = async () => {
    try {
      const blob = await exportRecordsMutation.mutateAsync();
      const fileName = `asset-records-${new Date().toISOString().split("T")[0]}.xlsx`;
              ...
              <div className="flex justify-between items-center mb-6">
                <div>
                  ...
                  >
                    ...
                    <span>•</span>
                    <span>
                      {new Date(selectedRecord.submissionDate).toLocaleString()}
                    </span>
                  </div>
