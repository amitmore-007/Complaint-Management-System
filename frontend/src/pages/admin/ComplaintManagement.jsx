                ...
                <div className="p-6">
                  ...
                  {selectedComplaint && (
                    <div className="mb-6">
                      ...
                      <p className={`text-sm ${
                        ...
                        Client: {selectedComplaint.client?.name} | Location: {selectedComplaint.location}
                      </p>
                    </div>
                  )}
                  ...
                  <div className="mb-6">
                    <label className={`block text-sm font-medium mb-2 ${
                      ...
                      Select Technician ({technicians.filter(t => t.isActive).length} available)
                    </label>
                    <select
                      ...
                      {technicians
                        ...
                        .map((tech) => (
                          <option key={tech._id} value={tech._id}>
                            {tech.name} ({tech.activeAssignments || 0} active, {tech.completedAssignments || 0} completed)
                          </option>
                        ))}
                    </select>
                  ...
                  <div className="space-y-6">
                    ...
                    <div>
                      <div className="flex items-center space-x-3 mb-3">
                        ...
                        <span className={`text-sm font-medium capitalize ${getPriorityColor(selectedComplaint.priority)}`}>
                    ...
                    {selectedComplaint.status === 'resolved' && (selectedComplaint.resolutionNotes || (selectedComplaint.resolutionPhotos && selectedComplaint.resolutionPhotos.length > 0)) && (
                      <div className={`p-4 rounded-lg border ${
                        isDarkMode ? 'bg-green-900/20 border-green-700' : 'bg-green-50 border-green-200'
                      }`}>
                        <h5 className={`font-medium mb-4 flex items-center space-x-2 ${
                        ...
                        {selectedComplaint.resolvedAt && (
                          <div className="mb-4">
                            <p className={`text-sm ${
                              ...
                              <strong>Completed on:</strong> {new Date(selectedComplaint.resolvedAt).toLocaleString()}
                            </p>
                          </div>
                        )}

                        {selectedComplaint.resolutionPhotos && selectedComplaint.resolutionPhotos.length > 0 && (
                          <div>
                            <h6 className={`font-medium mb-3 ${
                              isDarkMode ? 'text-green-200' : 'text-green-800'
                            }`}>
                              Resolution Proof Photos ({selectedComplaint.resolutionPhotos.length})
                            </h6>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                              {selectedComplaint.resolutionPhotos.map((photo, index) => (
                                <img
                                  key={index}
                                  src={photo.url}
