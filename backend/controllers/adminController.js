...
import { sendAssignmentNotification } from '../config/twilio.js';
...
export const assignComplaint = async (req, res) => {
  try {
    ...
    console.log('Assignment request:', { complaintId, technicianId, adminId });
    ...
    console.log('Technician found:', technician ? `${technician.name} (Active: ${technician.isActive})` : 'Not found');
    ...
    console.log('Complaint found:', complaint ? `${complaint.title} (Status: ${complaint.status})` : 'Not found');
...
export const autoAssignPendingComplaints = async () => {
  try {
    ...
    for (let i = 0; i < pendingComplaints.length; i++) {
      ...
      console.log(`Auto-assigned complaint ${complaint.complaintId} to ${selectedTech.name}`);
    }
  } catch (error) {
