...
const TechnicianDashboard = () => {
  ...
  const updateComplaintStatus = async (complaintId, newStatus, notes = '', resolutionData = null) => {
    try {
      ...
      if (newStatus === 'resolved' && (!resolutionData || !resolutionData.notes.trim())) {
        toast.error('Resolution notes are required when completing a complaint');
        return;
      }
                      ...
                      {complaint.status === 'resolved' && (complaint.resolutionNotes || (complaint.resolutionPhotos && complaint.resolutionPhotos.length > 0)) && (
                        <div className="p-3 bg-green-900/20 border border-green-700/50 rounded-lg">
                          <div className="flex items-center space-x-2 mb-2">
                            ...
                            <span className="text-xs sm:text-sm font-semibold text-green-300">Resolution Details</span>
                          </div>
                          ...
                          {complaint.resolvedAt && (
                            <p className="text-xs text-green-300">
                              <strong>Completed:</strong> {new Date(complaint.resolvedAt).toLocaleString()}
                            </p>
                          )}
                          ...
                          {complaint.resolutionPhotos && complaint.resolutionPhotos.length > 0 && (
                            <div className="mt-2">
                              <p className="text-xs text-green-300 mb-2">
                                Resolution Photos: {complaint.resolutionPhotos.length}
                              </p>
                              <div className="flex flex-wrap gap-2">
                                {complaint.resolutionPhotos.slice(0, 3).map((photo, index) => (
                                  <img
                                    key={index}
                                    src={photo.url}
                                    ...
                                    onClick={() => {
                                      ...
                                      const resolutionPhotoIndex = (complaint.photos?.length || 0) + index;
